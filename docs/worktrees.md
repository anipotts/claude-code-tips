...
### worktree.bgIsolation setting (v2.1.143+)

for repos where git worktrees are impractical (mono-repos, complex submodule layouts), set `bgIsolation: "none"` to let background sessions edit the working copy directly:
...
**caveat**: multiple background sessions can now conflict on the same files. use this only if your repo structure makes worktrees untenable. pair it with careful task scoping so background agents don't step on each other.
