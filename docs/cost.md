...
### note on version updates (v2.1.205+)

### note on version updates (v2.1.207+)

### note on version updates (v2.1.207+)

upstream claude code v2.1.207 is current as of july 2026. this document was last detailed verification with v2.1.122. before relying on cost figures, run `/lore` to check your actual per-session costs and caching behavior. caching efficiency, compaction rates, and token consumption patterns may have improved in recent versions. run `/doctor` to verify your billing method, token limits, and active feature set match the assumptions in this guide.
