...
hooks come in five flavors (v2.1.118+ added `mcp_tool`). pick the wrong one and you burn tokens, add latency, or silently fail.
...
| `mcp_tool` | directly invoke an MCP tool on a connected server (v2.1.118+) | 60s | free (no child process) | hook work that an MCP server already owns the state for |
...
stable since v2.1.118. now in v2.1.119+.
