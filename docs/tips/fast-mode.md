...
**note (v2.1.176+)**: fast mode now respects `enforceAvailableModels` allowlists. if fast mode would switch to a model outside your allowlist, the toggle is refused. this prevents bypassing model restrictions via fast-mode.
