...
**breaking change (v2.1.176)**: /fast now respects `availableModels` restrictions. if your settings define an allowlist and fast mode would switch to a blocked model, /fast refuses to toggle. this prevents bypassing model restrictions via fast-mode alias selection.
