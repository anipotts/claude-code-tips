...
**note (v2.1.116+):** `/resume` on large sessions is now 67% faster. if you're resuming a session instead of starting fresh, compaction pressure may be lower than these historical rates suggest. monitor your own compaction rate via `/mine` to see if patterns have shifted.
